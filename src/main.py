...
from commands import relink, genre, getPlaylist, addTrackToPlaylist
...
mqtt_topics = ['externalRequest', 'songReaction']
...
def subscribe(client: mqtt_client):
    def on_message(client, userdata, msg):
        ...
        if msg.topic == f"{environment}/songReaction":
            ...
            if payload['service'] == 'spotify-client':
                if payload['nowPlaying']['provider'] == 'spotify':
                    if payload['name'] == 'relink':
                        ...
                        if payload['client'] == 'goodbot-ttl' or payload['client'] == 'RVRB':
...
def run():
    print('Starting spotify-clinet version 3.0.1')
